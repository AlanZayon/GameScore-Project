...
import { PosterRail } from '@/components/game/poster-rail';
    ...
    <main className="space-y-8 py-6">
      ...
      {failed ? (
        <div className="container-page">
          <ErrorState title={t('emptySection')} />
        </div>
      ) : null}
      ...
      {sections.map((section, sectionIndex) => (
        <div key={section.key} className="space-y-8">
          {sectionIndex === 1 ? (
            <div className="container-page">
              <AdSlot format="horizontal" label="home-mid" />
            </div>
          ) : null}
          <PosterRail
            title={t(section.key)}
            seeAllHref={section.href}
            seeAllLabel={common('seeAll')}
            empty={t('emptySection')}
            games={section.games}
            priorityFirst={section.key === 'popular'}
          />
        </div>
      ))}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 space-y-0.5">
        ...
        {review.gameDeveloper ? (
          <p className="truncate text-xs text-content-muted">{review.gameDeveloper}</p>
        ) : null}
