...
export function GameMedia({
  ...
  if (!trailerYoutubeId && gallery.length === 0) return null;

  const current = gallery.length > 0 ? gallery[Math.min(index, gallery.length - 1)]! : null;
    ...
    <div className="space-y-5">
      {trailerYoutubeId ? (
        <section className="space-y-3">
          <h2 className="font-display text-lg font-semibold">{t('mediaTrailer')}</h2>
      ...
      {current ? (
        <section className="space-y-3">
          <h2 className="font-display text-lg font-semibold">{t('mediaGallery')}</h2>
